...
class UsuarioController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if (Yii::$app->user->identity->tipo !== 'administrador') {
            throw new \yii\web\ForbiddenHttpException('Não tem permissão para registar usuários.');
        }

        $model = new Usuario();

        if ($model->load(Yii::$app->request->post())) {
            $model->generateAuthKey(); // Gera o authKey automaticamente
            if ($model->save()) {
